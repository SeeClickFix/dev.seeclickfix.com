...
Endpoints that require authentication will return a HTTP 403 or 404. Some
non-authenticated endpoits will return additional information to authenticated
users so we advise that all api access be authenitcated.

### Basic Authentication

For individual access we allow http basic auth.

<pre class="terminal">
$ curl -u "emailaddress@example.com:password-would-go-here" <%= root_version_url %>/issues/new?address=New+Haven,+CT
</pre>
...
<pre class="terminal">
$ curl -H "Authorization: token OAUTH-TOKEN" <%= root_version_url %>/issues/new?address=New+Haven,+CT
</pre>

### OAuth2 Token (sent as a parameter)

<pre class="terminal">
curl <%= root_version_url %>/issues/new?address=New+Haven,+CT&access_token=OAUTH-TOKEN
</pre>

Read more about [OAuth2](http://oauth.net/2/) and available libraries. To obtain an OAuth2 app_id and secret please email us at <team@seeclickfix.com>.
...
At various endpoints SeeClickFix recognizes geography either by a `point` or `area` attribute. If multiple points/areas are specified, they will be evaulated in the listed order of precedence.
...
1. `llzoom=:latitude,:longitude,:zoom`
