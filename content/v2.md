...
1. `point=:latitude|:longitude`
1. `point=:address`

If the submitted value has a pipe, it will be evaluated as a point. Otherwise, we will geocode the address and use the resulting latitude/longitude as a point.

`area` attributes can be submitted in the following forms:

1. `area=pointzoom|:latitude,:longitude,:zoom`
1. `area=addresszoom|:address,:zoom`
1. `area=place|:url_name`
1. `area=boundingbox|:sw_lat,:sw_lng,:ne_lat,:ne_lng`
1. `area=watcharea|:watch_area_token`
